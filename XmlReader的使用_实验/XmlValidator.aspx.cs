...
namespace XmlReader的使用_实验_VS2012
{
	public partial class Default2 : Page
	{
		...
		private void Validate()
		{
			...
			TextReader schemaTextReader = new StringReader(schemaContent);
				//创建一个TextReader对象，用于读取右侧文本框中的文本值，并在创建XmlSchema对象时作为构造函数的参数传入
			...
			TextReader docTextReader = new StringReader(docContent);
				//创建另一个TextReader对象，用于读取左侧侧文本框中的文本值，并在创建XmlReader对象时作为构造函数的参数传入
